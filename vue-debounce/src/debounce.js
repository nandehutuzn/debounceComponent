...
const getEventHandle = (vnode, event) => {
    if(vnode) {
        let tmpVnode = vnode
        if(vnode.data && vnode.data.on && vnode.data.on[event]) {
            return {vnode: vnode, handle: vnode.data.on[event]}
        } else if(vnode.children && vnode.children.length > 0) {
            tmpVnode = vnode.children[0]
            return getEventHandle(tmpVnode, event)
        }
    }
    return null
}
        ...
        this.eventKeys.forEach(event => {
            ...
            if(target) { // render方法界面有更新就会执行，所以将设置好的方法放在缓存中，减少debounce方法执行次数
                if(target.handle === this.originMap[event] && this.debounceMap[event]){
                    target.vnode.data.on[event] = this.debounceMap[event]
